...
| V22 | Dedicated reproducible development environment | ✅ `horus` conda env (Python 3.12) installed from checked-in requirements; `make check` runs the exact current test/ruff/mypy gate without borrowing a sibling env |
| V23 | Joint air + sea fusion capstone | ✅ read-only HORUS×PHAROS coarse space-time correlator, graceful sibling failure, live 103-pair worked run, cross-lane ARGUS brief, and browser-verified UI honesty contract |
