...
HORUS deliberately avoids a heavy geo stack (PostGIS / GEOS / shapely) so the whole
...
def point_in_polygon(lat: float, lon: float, polygon: Sequence[tuple[float, float]]) -> bool:
    ...
    a planar test in lon/lat, which is fine at the zone sizes HORUS uses (not near the poles
    or the antimeridian).
    """
    if len(polygon) < 3:
        return False
